...
public class BrowserPoolTests
{
    ...
    public async Task An_account_that_is_working_does_not_hold_up_another_account()
    {
        ...
        await using (pool)
        {
            var held = pool.RunAsync(slow, s => s.RunAsync(Operation.Generate, "timeout", [], null,
            ...
            var result = await pool.RunAsync(free, s => s.RunAsync(Operation.Generate, "quick", [], null, (_, _) => Task.CompletedTask, default));
