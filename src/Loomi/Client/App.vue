...
async function session() { const s=await api('/api/session'); csrf=s.csrfToken; authenticated.value=s.authenticated; me.value=s.authenticated?{username:s.username,role:s.role,dailyQuota:s.dailyQuota}:null; devKey.value=s.devAccessKey||''; if(devKey.value && !authenticated.value && !password.value) { username.value='owner'; password.value=devKey.value; } }
...
<template>
 ...
 <div v-else class="app-shell">
  ...
  <aside :class="{open:mobileMenu}">
   ...
   <footer class="sidebar-footer"><button class="connection-button" @click="modal='connection'"><i class="status-dot" :class="{on:connected.state==='Connected'}"></i><span><strong>{{ t(connected.state) }}</strong><small>ChatGPT</small></span><span class="grow"/><ExternalLink/></button><div class="preferences"><button :class="{active:lang==='en'}" @click="lang='en'">EN</button><button :class="{active:lang==='fa'}" @click="lang='fa'">فا</button><span class="grow"/><button :aria-label="t('theme')" @click="theme=theme==='dark'?'light':'dark'"><Sun v-if="theme==='dark'"/><Moon v-else/></button></div><div class="owner"><span class="avatar"><ShieldCheck/></span><span><strong dir="auto">{{ me?.username || t('private') }}</strong><small>{{ me?.role ? t(me.role) : t('local') }}</small></span><span class="grow"/><button class="icon-button" :aria-label="t('logout')" @click="logout"><LogOut/></button></div></footer>
  </aside>
  <main>
 ...
 <Modal v-if="modal" :title="t(modal==='new'?'newProject':modal==='delete'?'delete':modal==='reset'?'reset':modal==='preview'?'preview':modal==='connection'?'connection':modal)" :close-label="t('close')" @close="modal=null">
  ...
  <div v-else-if="modal==='connection'"><div class="connection-status"><Monitor/><span><strong>{{ t(connected.state) }}</strong><small>{{ connected.busy?t('busy'):t('server') }}</small></span><i class="status-dot" :class="{on:connected.state==='Connected'}"></i></div><p>{{ t('connectionHelp') }}</p><button class="primary full" :disabled="busy||connected.busy" @click="connect"><RefreshCw/>{{ connected.state==='Disconnected'?t('connect'):t('reconnect') }}</button><a v-if="connected.state!=='Disconnected' && connected.desktopUrl" class="button full" :href="connected.desktopUrl" target="_blank" rel="noopener noreferrer"><ExternalLink/>{{ t('openBrowser') }}</a><p v-else-if="connected.state!=='Disconnected'" class="dev-note"><Monitor/><span>{{ t('desktopWindow') }}</span></p><p v-if="connected.state==='VerificationRequired'" class="dev-note"><ShieldCheck/><span>{{ t('verifyHelp') }}</span></p><p class="privacy-note"><ShieldCheck/>{{ t('sessionPrivacy') }}</p><footer class="modal-actions"><button class="danger" :disabled="busy||connected.busy" @click="modal='reset'">{{ t('reset') }}</button><button :disabled="busy" @click="action(refresh)"><RefreshCw/>{{ t('refresh') }}</button></footer></div>
