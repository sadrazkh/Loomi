...
const authenticated = ref(false), initialized = ref(false), accessKey = ref(''), devKey = ref(''), error = ref(''), busy = ref(false), view = ref('projects'), mobileMenu = ref(false);
...
async function session() { const s=await api('/api/session'); csrf=s.csrfToken; authenticated.value=s.authenticated; devKey.value=s.devAccessKey||''; if(devKey.value && !authenticated.value && !accessKey.value) accessKey.value=devKey.value; }
