...
| `GET /api/telegram/link` | وضعیت اتصال تلگرام شما؛ کد فعال اگر گرفته‌اید |
| `POST /api/telegram/link` | گرفتن کد اتصال ۸ نویسه‌ای با اعتبار ده دقیقه |
| `DELETE /api/telegram/link` | قطع اتصال تلگرام |

مسیرهای مالک (`/api/users`, `/api/auth/accounts`, `/api/pricing`) هم با توکنِ یک مالک کار می‌کنند، به جز مدیریت توکن‌ها.

## ربات تلگرام

اگر مالک `Telegram__BotToken` را تنظیم کرده باشد، هر کاربر می‌تواند یک گفت‌وگوی تلگرام را به حساب خود وصل کند: از تنظیمات کد بگیرید و در ربات `/start <code>` بفرستید. یک کاربر یک گفت‌وگو، و یک گفت‌وگو یک کاربر.

در ربات: متن ساده یک تصویر می‌سازد؛ عکس همراه با شرح، همان عکس را مرجع می‌گیرد؛ چند عکس که با هم فرستاده شوند یک درخواست با چند مرجع می‌شوند. `/balance` موجودی، `/status` کارهای در جریان، `/cancel` لغو کارهای ناتمام خودتان، `/help` راهنما.

کار ربات از همان `GenerationService` می‌گذرد که UI و API از آن می‌گذرند، پس اعتبار، سهمیه و قیمت‌گذاری یکسان است. همه‌ی کار ربات در پروژه‌ای به نام «Telegram» می‌نشیند و نتیجه فقط برای همان کارها به گفت‌وگو فرستاده می‌شود، نه برای کاری که در وب شروع کرده‌اید. مدیریت اتصال — مثل مدیریت توکن — فقط از نشست واردشده ممکن است، نه با توکن API.
...
| 409 | `AlreadyLinked` / `TelegramNotConfigured` | حسابتان از قبل به گفت‌وگویی وصل است، یا کارگاه ربات تلگرام ندارد |
